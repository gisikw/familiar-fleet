...
<body>{{if eq .Kind "form"}}<h1>Connect this machine</h1><form method="post" action="/enroll"><input type="hidden" name="state" value="{{.State}}"><label for="name">Machine name</label><input id="name" name="name" required maxlength="253" pattern="[A-Za-z0-9][A-Za-z0-9._-]{0,252}" value="{{.Name}}" autofocus><button type="submit">Connect</button></form><small>Letters, digits, dots, underscores, and hyphens.</small>{{else if eq .Kind "success"}}<h1>Connected</h1><p>This machine is connected; you can close this tab.</p>{{else}}<h1>Could not connect</h1><p class="error">{{.Message}}</p>{{end}}</body></html>`))
